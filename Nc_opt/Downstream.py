...
def test(model, loss_fn, train_dataloader, test_dataloader, device, scaler, optimizer, scheduler, epoch):
    ...
    # Storage for predictions, true values, and embeddings
    train_pred, train_true, test_pred, test_true = torch.tensor([]), torch.tensor([]), torch.tensor([]), torch.tensor([])
    train_embeddings, test_embeddings = [], []  # To store embeddings
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            # Update predictions and true values
    ...
    # Save embeddings as CSV files
    train_embeddings = np.vstack(train_embeddings)
    test_embeddings = np.vstack(test_embeddings)

    train_df = pd.DataFrame(train_embeddings)
    train_df["prop"] = train_true.cpu().numpy()
    train_df.to_csv(f"train_embeddings_epoch_{epoch}.csv", index=False)

    test_df = pd.DataFrame(test_embeddings)
    test_df["prop"] = test_true.cpu().numpy()
    test_df.to_csv(f"test_embeddings_epoch_{epoch}.csv", index=False)
