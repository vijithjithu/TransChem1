...
def roberta_base_AdamW_LLRD(model, lr, weight_decay):
    ...
    # === Regressors ======================================================
    for head_name in ["Eea_FCNN", "Egb_FCNN", "Egc_FCNN", "Ei_FCNN"]:
...
class MultiTask(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, new_feats):
        ...
        Eea_FCNN_output = self.Eea_FCNN(combined)  # Shape: [batch_size, 1]
        Egb_FCNN_output = self.Egb_FCNN(combined)  # Shape: [batch_size, 1]
        Egc_FCNN_output = self.Egc_FCNN(combined)  # Shape: [batch_size, 1]
        Ei_FCNN_output = self.Ei_FCNN(combined)    # Shape: [batch_size, 1]

        final_output = torch.cat([Eea_FCNN_output, Egb_FCNN_output, Egc_FCNN_output, Ei_FCNN_output], dim=1)  # Shape: [batch_size, 4]
...
def main(multitask_config):
    ...
    if multitask_config['CV_flag']:
        ...
        print("No. of features:",len(data.columns[1:-4]))
        print()
        """K-fold"""
        ...
        for fold, (train_idx, val_idx) in enumerate(splits.split(np.arange(data.shape[0]))):
            ...
            if multitask_config['normalize']:
                ...
                test_data.loc[:, feats_to_norm] = scaler_feats.transform(test_data.loc[:, feats_to_norm]).astype(test_data[feats_to_norm].dtypes)
            else:
                print("All features are normalized")
                scaler_feats = StandardScaler()
                train_data.iloc[:, 1:-4] = scaler_feats.fit_transform(train_data.iloc[:, 1:-4])
                test_data.iloc[:, 1:-4] = scaler_feats.transform(test_data.iloc[:, 1:-4])
