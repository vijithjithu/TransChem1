...
from newFeats_multi_dataset_nc import MultiTask_Dataset, MultiTask_DataAugmentation
...
class MultiTask(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, new_feats):
        ...
        combined = transformed_logits  # Shape: [batch_size, 32 + 73]

        
        EPS_FCNN_output = self.EPS_FCNN(combined)
        Nc_FCNN_output = self.Nc_FCNN(combined)

        final_output = torch.cat([EPS_FCNN_output, Nc_FCNN_output], dim=1)  # Shape: [batch_size, 4]
    ...
    if multitask_config['CV_flag']:
        ...
        for fold, (train_idx, val_idx) in enumerate(splits.split(np.arange(data.shape[0]))):
            ...
            if multitask_config['normalize']:
                ...
                print(('feats to norm', len(feats_to_norm)))
                ...
                train_data.iloc[:, feats_to_norm] = scaler_feats.fit_transform(train_data.iloc[:, feats_to_norm])
                test_data.iloc[:,feats_to_norm] = scaler_feats.transform(test_data.iloc[:, feats_to_norm])
