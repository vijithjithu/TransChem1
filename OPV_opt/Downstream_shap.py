...
class DownstreamRegression(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, return_embeddings=False):
        ...
        transformed_embeddings = self.embedding_transform(embeddings)

        

        if return_embeddings:

            return None, transformed_embeddings


        output = self.Regressor(embeddings)
        return output, transformed_embeddings
...
def test(model, loss_fn, train_dataloader, test_dataloader, device, scaler, optimizer, scheduler, epoch):
    ...
    test_loss = 0
    model.eval()
    all_embeddings = []  # To store embeddings for analysis

    with torch.no_grad():
        train_pred, train_true, test_pred, test_true = torch.tensor([]), torch.tensor([]), torch.tensor([]), torch.tensor([])
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            outputs = torch.from_numpy(scaler.inverse_transform(output.cpu().reshape(-1, 1)))
        ...
        for step, batch in enumerate(test_dataloader):
            ...
            output, embeddings = model(input_ids, attention_mask)  # Unpack output
            outputs = torch.from_numpy(scaler.inverse_transform(output.cpu().reshape(-1, 1)))
            prop = torch.from_numpy(scaler.inverse_transform(prop.cpu().reshape(-1, 1)))
            loss = loss_fn(outputs.squeeze(), prop.squeeze())
    ...
    # Return embeddings along with loss and r^2 values
    return train_loss, test_loss, r2_train, r2_test, all_embeddings
...
def main(finetune_config):
    ...
    if finetune_config['CV_flag']:
        ...
        splits = KFold(n_splits=finetune_config['k'], shuffle=True, random_state=1)
        train_loss_avg, test_loss_avg, train_r2_avg, test_r2_avg = [], [], [], []
        
        # Track embeddings from the best fold and epoch
        ...
        for fold, (train_idx, val_idx) in enumerate(splits.split(np.arange(data.shape[0]))):
            ...
            count = 0  # Track successive non-improvement epochs
            for epoch in range(finetune_config['num_epochs']):
                ...
                train_loss, test_loss, r2_train, r2_test, all_embeddings = test(model, loss_fn, train_dataloader,
                ...
                if r2_test > best_test_r2:
                    ...
                    count = 0
                    # Track embeddings for the best epoch
            ...
            # After each fold, save embeddings from the best epoch
            if best_embeddings:
                pd.DataFrame(best_embeddings).to_csv(f"embeddings_fold_{fold+1}.csv", index=False)
