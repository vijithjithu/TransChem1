...
class DownstreamRegression(nn.Module):
    def __init__(self, drop_rate=0.1):
        ...
        self.embedding_transform = nn.Linear(self.PretrainedModel.config.hidden_size, 768)

        self.Regressor = nn.Sequential(nn.Linear(self.PretrainedModel.config.hidden_size+22, 512),
            nn.BatchNorm1d(512),
            nn.SiLU(),
            nn.Linear(512,256),
            nn.SiLU(),
    ...
    # Get embeddings from PretrainedModel
        outputs = self.PretrainedModel(input_ids=input_ids, attention_mask=attention_mask)
        logits = outputs.last_hidden_state[:, 0, :]  # CLS token embeddings
        
        # Transform embeddings
        transformed_logits = self.embedding_transform(logits)
        
        # Combine features
        input_to_regressor = torch.cat([transformed_logits, new_feats], dim=1)  # Make sure this is called correctly
        
        # Save combined features as a DataFrame
        combined_feat = pd.DataFrame(input_to_regressor.detach().cpu().numpy())  # Convert to NumPy array
        combined_feat.to_csv('combined_feat.csv', index=False)  # Save to CSV
...
def test(model, loss_fn, train_dataloader, test_dataloader, device, scaler_target, optimizer, scheduler, epoch):
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            all_train_pred.extend(outputs.cpu().numpy().flatten())  # Store predictions
            all_train_true.extend(prop.cpu().numpy().flatten())  # Store true values
        ...
        for step, batch in enumerate(test_dataloader):
            ...
            all_test_pred.extend(outputs.cpu().numpy().flatten())  # Store predictions
            all_test_true.extend(prop.cpu().numpy().flatten())  # Store true values
    ...
    print(f"\nTrain Predictions vs True Values (as lists)")
    print("True Values:", all_train_true)
    print("Predicted Values:", all_train_pred)

    print("\nTest Predictions vs True Values (as lists):")
    print("True Values:", all_test_true)
    print("Predicted Values:", all_test_pred)
...
def main(finetune_config):
    ...
    f = open('Ei_chem_feature_names.json')
...
if __name__ == "__main__":
    ...
    finetune_config = yaml.load(open("config_Ei_TransChem_finetune_params.yaml", "r"), Loader=yaml.FullLoader)
