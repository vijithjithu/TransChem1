...
from new_feat_data import Downstream_Dataset, DataAugmentation
...
class DownstreamRegression(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, anion_smile_encoding, log_li, comonomer_percentage, approxMW, approxTg, chain_architecture, new_feats):
        outputs = self.PretrainedModel(input_ids=input_ids, attention_mask=attention_mask)
        logits = outputs.last_hidden_state[:, 0, :]
        smile_embeddings = self.smile_autoencoder(logits)
        input_to_regressor = torch.cat([smile_embeddings, anion_smile_encoding, log_li.unsqueeze(1), comonomer_percentage.unsqueeze(1), approxMW.unsqueeze(1), approxTg.unsqueeze(1), chain_architecture.unsqueeze(1), new_feats], dim=1).clone().detach()
...
def train(model, optimizer, scheduler, loss_fn, train_dataloader, device):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        outputs = model(input_ids, attention_mask, anion_smile_encoding, log_li,
...
def main(finetune_config):
    ...
    if finetune_config['CV_flag']:
        ...
        scaler_target = StandardScaler()
        train_data.iloc[:, -1] = scaler_target.fit_transform(
            train_data.iloc[:, -1].values.reshape(-1, 1))
        test_data.iloc[:, -1] = scaler_target.transform(
