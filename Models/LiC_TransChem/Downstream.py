...
def test(model, loss_fn, train_dataloader, test_dataloader, device, scaler, optimizer, scheduler, epoch):
    ...
    with torch.no_grad():
        ...
        mae_test = mae(test_pred.flatten().to("cpu"), test_true.flatten().to("cpu")).item()
...
def main(finetune_config):
    ...
    if finetune_config['CV_flag']:
        ...
        dataset = pd.read_csv(finetune_config['dataset_file'])
        ...
        #seeds = [22]
        print("Seed 94")
        gss = GroupShuffleSplit(n_splits=1,train_size=0.8,random_state=94)
        ...
        train_data = train_data.drop(columns=['Polymer'])
        test_data = test_data.drop(columns=['Polymer'])
