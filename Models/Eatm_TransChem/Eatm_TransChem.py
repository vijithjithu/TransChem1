...
class DownstreamRegression(nn.Module):
    def __init__(self, drop_rate=0.1):
        ...
        self.Regressor = nn.Sequential(nn.Linear(32 +16, 32),
                                       nn.LayerNorm(32),
                                       nn.Dropout(0.1),
                                       nn.SiLU(),
                                        nn.Linear(32,1)
    ...
    def forward(self, input_ids, attention_mask, new_feats):
        ...
        return output, input_to_regressor
...
def train(model, optimizer, scheduler, loss_fn, train_dataloader, device):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        optimizer.zero_grad()
        outputs, combined_embeddings = model(input_ids, attention_mask, new_feats)  
        outputs = outputs.float() 

        all_combined_embeddings.append(combined_embeddings.detach().cpu().numpy())
    ...
    all_combined_embeddings = np.concatenate(all_combined_embeddings, axis=0) 

    
    return None
def test(model, loss_fn, train_dataloader, test_dataloader, device, scaler_target, optimizer, scheduler, epoch,best_test_r2):
    r2score = R2Score()
    train_loss, test_loss = 0, 0
    all_train_pred, all_train_true = [], []
    all_test_pred, all_test_true = [], []
    all_train_combined_embeddings, all_test_combined_embeddings = [], []  
    ...
    with torch.no_grad():
        for step, batch in enumerate(train_dataloader):
            ...
            outputs, combined_embeddings_tr = model(input_ids, attention_mask, new_feats)  
            outputs = outputs.float()
            ...
            all_train_pred.extend(outputs.cpu().numpy().flatten())
            all_train_true.extend(prop.cpu().numpy().flatten())
            all_train_combined_embeddings.append(combined_embeddings_tr.detach().cpu().numpy())
        
        train_loss /= len(all_train_pred)
        r2_train = r2score(torch.tensor(all_train_pred), torch.tensor(all_train_true)).item()
        print(f"Epoch {epoch} - Train RMSE: {np.sqrt(train_loss)}, R²: {r2_train}")
        ...
        for step, batch in enumerate(test_dataloader):
            ...
            outputs, combined_embeddings_te = model(input_ids, attention_mask, new_feats)  
            outputs = outputs.float()
            outputs = torch.from_numpy(scaler_target.inverse_transform(outputs.cpu().reshape(-1, 1)))
            prop = torch.from_numpy(scaler_target.inverse_transform(prop.cpu().reshape(-1, 1)))
            ...
            all_test_pred.extend(outputs.cpu().numpy().flatten())
            all_test_true.extend(prop.cpu().numpy().flatten())
            all_test_combined_embeddings.append(combined_embeddings_te.detach().cpu().numpy())
        
        test_loss /= len(all_test_pred)
        r2_test = r2score(torch.tensor(all_test_pred), torch.tensor(all_test_true)).item()
        ...
        if r2_test > best_test_r2:
            ...
            all_train_combined_embeddings = np.concatenate(all_train_combined_embeddings, axis=0)
            all_test_combined_embeddings = np.concatenate(all_test_combined_embeddings, axis=0)

            #pd.DataFrame(all_train_combined_embeddings).to_csv(f'train_combined_embeddings_best_epoch.csv', index=False)
            #pd.DataFrame(all_test_combined_embeddings).to_csv(f'test_combined_embeddings_best_epoch.csv', index=False)
            #pd.DataFrame({'True_Values': all_train_true}).to_csv(f'train_true_values_best_epoch.csv', index=False)
            #pd.DataFrame({'True_Values': all_test_true}).to_csv(f'test_true_values_best_epoch.csv', index=False)
    print('saved')
...
def main(finetune_config):
    ...
    f = open('Eatm_chem_feature_names.json')
    ...
    for see in seed:
        ...
        if finetune_config['CV_flag']:
            ...
            for epoch in range(finetune_config['num_epochs']):
                ...
                train_loss, test_loss, r2_train, r2_test = test(model, loss_fn, train_dataloader,
                                                                                    test_dataloader, device, scaler_target,
                                                                                    optimizer, scheduler, epoch,best_test_r2)
                if r2_test > best_test_r2:
                    best_train_r2 = r2_train
                    best_test_r2 = r2_test
                    train_loss_best = train_loss
...
if __name__ == "__main__":
    ...
    finetune_config = yaml.load(open("config_Eatm_TransChem_finetune_params.yaml", "r"), Loader=yaml.FullLoader)
