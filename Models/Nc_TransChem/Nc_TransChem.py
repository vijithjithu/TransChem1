...
class DownstreamRegression(nn.Module):
    def __init__(self, drop_rate=0.1):
        ...
        self.Regressor = nn.Sequential(nn.Linear(self.PretrainedModel.config.hidden_size+20,256),
            nn.BatchNorm1d(256),
            nn.SiLU(),
            nn.Dropout(0.3976),
            nn.Linear(256,64),
    ...
    def forward(self, input_ids, attention_mask, new_feats):
        ...
        input_to_regressor = torch.cat([logits, new_feats], dim=1).clone().detach()
...
def main(finetune_config):
    ...
    f = open('Nc_chem_feature_names.json')
...
if __name__ == "__main__":
    ...
    finetune_config = yaml.load(open("config_Nc_TransChem_finetune_params.yaml", "r"), Loader=yaml.FullLoader)
