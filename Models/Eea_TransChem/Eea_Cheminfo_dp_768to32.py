...
class DownstreamRegression(nn.Module):
    def __init__(self, drop_rate=0.1):
        ...
        #    nn.Dropout(drop_rate),
         #   nn.Linear(self.PretrainedModel.config.hidden_size, self.PretrainedModel.config.hidden_size),
          #  nn.SiLU(),
            nn.Linear(self.PretrainedModel.config.hidden_size, 768)
        )

        self.Regressor = nn.Sequential(nn.Linear(768+29, 512),
            nn.BatchNorm1d(512),
            nn.SiLU(),
            nn.Dropout(drop_rate),
            nn.Linear(512,256),
    ...
    def forward(self, input_ids, attention_mask, new_feats):
        ...
        input_to_regressor = torch.cat([smile_embeddings, new_feats], dim=1).clone().detach()
...
def test(model, loss_fn, train_dataloader, test_dataloader, device, scaler_target, optimizer, scheduler, epoch, fold):
    r2score = R2Score()
    train_loss = 0
    test_loss = 0
    model.eval()
    ...
    with torch.no_grad():
        # Collect predictions and true values for training set
        for step, batch in enumerate(train_dataloader):
            input_ids = batch["input_ids"].to(device)
            attention_mask = batch["attention_mask"].to(device)
            new_feats = batch["new_feats"].to(device)
        ...
        train_loss = train_loss / len(all_train_pred)
        r2_train = r2score(torch.tensor(all_train_pred), torch.tensor(all_train_true)).item()

        print(f"Fold {fold} Train RMSE = ", np.sqrt(train_loss))
        print(f"Fold {fold} Train r^2 = ", r2_train)

        # Collect predictions and true values for test set
        ...
        test_loss = test_loss / len(all_test_pred)
        r2_test = r2score(torch.tensor(all_test_pred), torch.tensor(all_test_true)).item()

        print(f"Fold {fold} Test RMSE = ", np.sqrt(test_loss))
        print(f"Fold {fold} Test r^2 = ", r2_test)

    return train_loss, test_loss, r2_train, r2_test, all_train_pred, all_train_true, all_test_pred, all_test_true



def main(finetune_config):

    f = open('Eea_chem_feature_names.json')
            ...
            for fold, (train_idx, val_idx) in enumerate(splits.split(np.arange(data.shape[0]))):
                ...
                for epoch in range(finetune_config['num_epochs']):
                    ...
                    train_loss, test_loss, r2_train, r2_test = test(model, loss_fn, train_dataloader,
                                                                                    ...
                                                                                    optimizer, scheduler, epoch,fold)
                ...
                print(f'Best test loss of fold{fold+1}',np.sqrt(test_loss_best))

                print(f'Best test R2 of fold{fold+1}',best_test_r2)
...
if __name__ == "__main__":
    ...
    finetune_config = yaml.load(open("config_Eea_TransChem_finetune_params.yaml", "r"), Loader=yaml.FullLoader)
